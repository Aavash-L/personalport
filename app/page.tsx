...
export default function Home() {
  ...
  return <>
    ...
    <section id="about" className="section about-section"><span className="eyebrow">03 / INTRODUCTION</span><div className="about-grid"><h2>{SITE.aboutTitle}</h2><div className="bio">{BIO.map(p=><p key={p}>{p}</p>)}<p className="education">{SITE.education}</p></div></div><div className="stats">{STATS.map(s=><div key={s.label}><strong>{s.value}</strong><span>{s.label}</span></div>)}</div></section>
