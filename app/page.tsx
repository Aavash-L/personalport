...
export default function Home() {
  ...
  return <>
    ...
    <section id="contact" className="section contact-section"><span className="eyebrow">05 / WHAT’S NEXT</span><h2>{SITE.contactTitle}</h2><p>{SITE.contactText}</p><div className="contact-actions"><a className="button primary" href={CONTACT_LINKS[0].href}>Email Aavash <span>↗</span></a><a className="button" href={SITE.resume} target="_blank" rel="noopener noreferrer">Download resume <span>↓</span></a></div><div className="contact-links">{CONTACT_LINKS.map(l=><a key={l.platform} href={l.href}>{l.platform} <span>↗</span></a>)}</div></section>
